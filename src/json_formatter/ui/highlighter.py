...
class JsonSyntaxHighlighter(QSyntaxHighlighter):
    def __init__(self, parent) -> None:
        ...
        self.apply_theme(get_theme("system"))

    def apply_theme(self, theme: ThemeSpec) -> None:
        self._brace_format = self._make_format(theme.syntax_brace, bold=True)
        self._key_format = self._make_format(theme.syntax_key, bold=True)
        self._string_format = self._make_format(theme.syntax_string)
        self._number_format = self._make_format(theme.syntax_number)
        self._literal_format = self._make_format(theme.syntax_literal, bold=True)
        self.rehighlight()
